...
async function addSlots(db: Db, garageId: number, daysAhead = 10, startHour = 10, endHour = 19, durationMin = 60) {
...
async function recalcMasterRating(db: Db, masterUserId: number) {
  const rating = await db.get<{ avg: string | number | null; count: string | number }>(
    `
...
export async function seed(db: Db) {
  ...
  const serviceRows = await db.all<{ id: number; category: string; name: string }>("SELECT id, category, name FROM services");
  ...
  const adminRes = await db.run(
    "INSERT INTO users (role, email, password_hash, personal_data_agreed, personal_data_agreed_at, created_at) VALUES ('ADMIN', ?, ?, 1, ?, ?) RETURNING id",
    ["admin@example.com", passAdmin, now, now]
  );
  await db.run(
    "INSERT INTO user_profiles (user_id, display_name, about, avatar_url, city, car_info, updated_at) VALUES (?, 'Администратор', 'Проверяет карточки мастерских и следит за качеством каталога.', '', 'Ульяновск', '', ?) RETURNING user_id",
    [Number(adminRes.lastInsertRowid), now]
  );

  const passUser = hashPassword("user123");
  const userRes = await db.run(
    "INSERT INTO users (role, email, password_hash, personal_data_agreed, personal_data_agreed_at, created_at) VALUES ('USER', ?, ?, 1, ?, ?) RETURNING id",
    ["user@example.com", passUser, now, now]
  );
  const demoUserId = Number(userRes.lastInsertRowid);
  await db.run(
    "INSERT INTO user_profiles (user_id, display_name, about, avatar_url, city, car_info, updated_at) VALUES (?, 'Алексей', 'Люблю понятный ремонт без лишних замен. Ищу мастеров по электрике, подвеске и обслуживанию.', '/images/master-sergey.jpg', 'Ульяновск', 'Lada Priora 2012', ?) RETURNING user_id",
    [demoUserId, now]
  );
  ...
  const masters = [
    {
      ...
      garage: {
        ...
        description:
          "Рабочий гаражный бокс без лишнего пафоса: диагностика, электрика, ремонт проводки, поиск пропадающего питания, установка допоборудования. Есть подъёмник, диагностический сканер и нормальный ручной инструмент.",
        services: [
    ...
    },
    {
      ...
      garage: {
        ...
        description:
          "Небольшая частная мастерская для ремонта подвески, тормозов и базового обслуживания. Упор на понятную диагностику и честное объяснение, что менять сейчас, а что можно отложить.",
        services: [
    ...
    },
    {
      ...
      garage: {
        ...
        description:
          "Кузовные работы в гаражном формате: сварка, рихтовка, подготовка элементов и локальная покраска. Подходит для тех, кому нужен не красивый ресепшен, а нормальный результат по делу.",
        services: [
    ...
    },
    {
      ...
      garage: {
        ...
        description:
          "Гараж для нестандартных задач: классические Лады, подвеска, мелкий тюнинг, подготовка авто к зимней езде. В проекте показан как пример карточки с яркой специализацией.",
        services: [
  ...
  for (const m of masters) {
    const userRes = await db.run(
      "INSERT INTO users (role, email, password_hash, personal_data_agreed, personal_data_agreed_at, created_at) VALUES ('MASTER', ?, ?, 1, ?, ?) RETURNING id",
      [m.email, masterPass, now, now]
    );
    const masterId = Number(userRes.lastInsertRowid);
    await db.run(
      "INSERT INTO user_profiles (user_id, display_name, about, avatar_url, city, car_info, updated_at) VALUES (?, ?, ?, ?, 'Ульяновск', '', ?) RETURNING user_id",
      [masterId, m.name, m.about, m.avatar, now]
    );
    ...
    for (const [serviceName, price, duration] of g.services) {
      ...
      if (sid) {
        if (!firstServiceId) firstServiceId = sid;
    ...
    for (let i = 0; i < g.demoReviews.length; i++) {
      ...
      const bookingRes = await db.run(
        "INSERT INTO bookings (user_id, garage_id, service_id, slot_start, slot_end, status, created_at) VALUES (?, ?, ?, ?, ?, 'DONE', ?) RETURNING id",
        [demoUserId, garageId, firstServiceId, slotStart, slotEnd, slotStart - 3600 * 1000]
      );
  ...
  await db.run(
    "INSERT INTO notifications (user_id, type, title, text, link, created_at) VALUES (?, 'SYSTEM', 'Добро пожаловать в GarageMaster', 'Здесь будут появляться заявки, модерация, статусы и отзывы.', '/me', ?) RETURNING id",
    [demoUserId, now]
  );
