...
export function registerFavoriteRoutes(app: FastifyInstance) {
  ...
  app.post("/api/favorites/:garageId", async (req, reply) => {
    const auth = await requireAuth(req, reply);
    if (!auth) return;
    const garageId = z.coerce.number().int().positive().parse((req.params as any).garageId);
    const garage = await app.db.get<{ id: number }>("SELECT id FROM garages WHERE id=? AND is_approved=1", [garageId]);
    if (!garage) return reply.code(404).send({ ok: false, error: "Гараж не найден" });
    await app.db.run(
      "INSERT INTO favorite_garages (user_id, garage_id, created_at) VALUES (?, ?, ?) ON CONFLICT (user_id, garage_id) DO NOTHING",
      [auth.sub, garageId, Date.now()]
    );
    return { ok: true };
  });
