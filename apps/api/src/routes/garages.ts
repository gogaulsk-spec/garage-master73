...
function normalizeGarageRow(row: any) {
  if (!row) return row;
...
type GeocodeResult = { lat: number; lng: number; displayName: string; source: string };
...
async function createSlots(db: Db, garageId: number, opts: { daysAhead: number; startHour: number; endHour: number; slotDurationMin: number }) {
  const startDay = normalizeStartDay(Date.now());
  const existingRows = await db.all<{ start_at: string | number; end_at: string | number }>(
    "SELECT start_at, end_at FROM availability_slots WHERE garage_id=? AND start_at >= ?",
    [garageId, startDay.getTime()]
  );
  ...
  for (let d = 0; d < opts.daysAhead; d++) {
    ...
    for (
      ...
      cursor + opts.slotDurationMin * 60 * 1000 <= to.getTime();
      cursor += opts.slotDurationMin * 60 * 1000
    ) {
...
export function registerGarageRoutes(app: FastifyInstance) {
  ...
  app.get("/api/geocode", async (req, reply) => {
    const q = z.object({ address: z.string().trim().min(5, "Укажи адрес") }).parse((req.query ?? {}) as any);
    const result = await geocodeAddress(q.address);
    if (!result) return reply.code(404).send({ ok: false, error: "Не удалось найти координаты по адресу" });
    ...
    const result = await app.db.transaction(async (tx) => {
      ...
      for (const item of body.services) {
        await tx.run(
          ...
          [garageId, item.serviceId, item.priceFrom ?? null, item.durationMin ?? body.schedule.slotDurationMin]
        );
      }
  ...
  app.patch("/api/master/garages/:id/schedule", async (req, reply) => {
    ...
    const garage = await app.db.get<any>("SELECT id FROM garages WHERE id=? AND master_user_id=?", [id, auth.sub]);
  ...
  app.get("/api/garages/:id", async (req, reply) => {
    const id = Number((req.params as any).id);
    const garage = normalizeGarageRow(
      await app.db.get(
        `
          SELECT
    ...
    if (!garage) return reply.code(404).send({ ok: false, error: "Not found" });
    ...
    const slots = (await app.db.all(
      ...
      `,
      [id, Date.now()]
    )).map((slot: any) => ({ ...slot, isBooked: Number(slot.isBooked) === 1 }));
  ...
  app.patch("/api/admin/garages/:id/moderation", async (req, reply) => {
    ...
    if (res.changes !== 1) return reply.code(404).send({ ok: false, error: "Гараж не найден" });
